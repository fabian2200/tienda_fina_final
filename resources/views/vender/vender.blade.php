...
<h1 style="width: 100%; text-align: left; color: red"><strong>Esta vendiendo como:  ({{session('tipo_usuario')}})</strong></h1>
    ...
    <div class="modal fade" id="modalCopias" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content">
            <br>
            <form id="guardarCopias" method="post">
                @csrf
                <div class="modal-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 style="background-color: rgb(243, 221, 122); padding: 5px; width: fit-content;">Ingrese el valor del producto</h2>
                        </div>
                    </div>
                    <input id="codigo_barras_impresion" name="codigo" autocomplete="off" required value="" type="hidden"class="form-control">
                    <br>
                    <div class="row">
                        <div class="col-lg-12">
                            <label style="font-size: 25px; font-weight: bold" for="cantidad_manual_peso">Valor del producto</label>
                            <input autocomplete="off" style="font-size: 25px; font-weight: bold" required  name="cantidad" type="number" id="cantidad_copia" class="form-control" placeholder="ej: 1200">
                        </div>
                    </div>
                </div>
                <div class="modal-footer" style="justify-content: center">
                    <button onclick="agregarCopiaImpresion()" type="button" class="btn_modal btn btn-warning">Guardar</button>
                    <button type="button" data-dismiss="modal" class="btn_modal btn btn-danger">Cerrar</button>
                </div>
            </form>
          </div>
    ...
    <script>
        ...
        function verificarUnidad(){
            ...
            if (codigo === '') {
                ...
                if(codigo=="IC2024" || codigo=="OT2024"){
                    $('#modalCopias').modal("show");
                    document.getElementById("codigo_barras_impresion").value = codigo;
                    return false;
                }else{
                    $.ajax({
        ...
        function agregarCopiaImpresion(){
            var cantidad = document.getElementById("cantidad_copia").value;
            var data_enviar = $('#guardarCopias').serialize();

            if(cantidad == ""){
                Swal.fire({
                    position: "center",
                    icon: "error",
                    title: "Ingrese el precio de las copias o impresiones",
                ...
                $.ajax({
                    ...
                    data: data_enviar, 
                    success: function(response) {
                        $("#cantidad_copia").val("").focus();
                        if(response.status != "error"){
                            location.reload();
                        }else{
                            $('#modalCopias').modal("hide");
                   ...
                   total = response.total;
                    ...
                    productos.forEach(function(producto, index) {
                        ...
                        if (producto.unidad_medida == "Unidades") {
                            ...
                            row += '<input style="width: 100px; font-size: 18px" class="form-control" disabled type="text" value="' + producto.cantidad + ' ' + (producto.unidad_medida == "Kilos" ? "Kg" : (producto.unidad_medida == "Libras" ? "Lb" : (producto.unidad_medida == "Gramos" ? "Grm" : "Und"))) + '">';
                        }
