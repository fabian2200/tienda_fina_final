...
class VenderController extends Controller{
        ...
        if($tipo_venta != 1){
            ...
            foreach ($productos as $producto) {
                ...
                $productoVendido->fill([
                    ...
                    "unidad" => $producto->unidad_medida == "Kilos" ? "Kg" : ($producto->unidad_medida == "Libras" ? "Lb" : ($producto->unidad_medida == "Gramos" ? "Grm" : "Und"))
                ]);
    ...
    public function productoCopias(Request $request){        
        $codigo = $request->post("codigo");
        $precio = $request->post("cantidad");
        $cantidad = 1;
        ...
        $respuesta = $this->agregarCopiasACarrito($producto, $cantidad, $precio);

        return $respuesta;
    }


    private function agregarCopiasACarrito($producto, $cantidad, $precio){
        $productos = $this->obtenerProductos();

        $producto->cantidad = $cantidad;
        $producto->precio_total = self::redondearAl100($precio);
        $producto->precio_venta = self::redondearAl100($precio);
        array_unshift($productos, $producto);
